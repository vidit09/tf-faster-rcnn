...
def grocery_eval(detpath,
    ...
    fp = np.cumsum(fp)
    tp = np.cumsum(tp)
    rec = tp / float(np.maximum(npos, np.finfo(np.float64).eps))
...
def grocery_eval_eccv_14(detpath,
    ...
    for imagename in imagenames:
        ...
        ids = [i for i,name in enumerate(image_ids) if imagename == name]
        confidence = np.array([float(splitlines[id][1]) for id in ids])
        BB = np.array([[float(z) for z in splitlines[id][2:]] for id in ids])
        ...
        fp = np.zeros(nd)
        for d in range(nd):
            dR = class_recs[imagename]
            bb = BB[d, :].astype(float)
            ovmax = -np.inf
            BBGT = dR['bbox'].astype(float)
